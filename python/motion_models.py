...
def variance_distance_travelled_s(distance):
    ...
    var_s = 0.32293*distance**2 + 0.014545
    return var_s
...
# changed to function of steer cmd w -caitlin
def variance_rotational_velocity_w(steering_angle_command):
    # Add student code here
    var_w = 0.18985*steering_angle_command**2 + 2.938*steering_angle_command + 11.3665
    var_w = math.radians(var_w)
    return var_w
...
class MyMotionModel:
    ...
    def step_update(self, encoder_counts, steering_angle_command, delta_t):
        ...
        delta_counts = encoder_counts - self.last_encoder_count
        self.last_encoder_count = encoder_counts

        delta_s = distance_travelled_s(delta_counts)
        ...
        theta = self.state[2]
        #theta = math.radians(self.state[2])

        self.state[0] += v*math.cos(theta)*delta_t   # x_t
        self.state[1] += v*math.sin(theta)*delta_t   # y_t
        self.state[2] += delta_t*w                   # theta_t

...
class UDPCommunication:
    def __init__(self, arduinoIP, localIP, arduinoPort, localPort, bufferSize):
        ...
        self.UDPServerSocket = socket.socket(family=socket.AF_INET, type=socket.SOCK_DGRAM)
        self.UDPServerSocket.bind((localIP, localPort))
        self.UDPServerSocket.settimeout(0.01)  # 10 ms timeout
    ...
    def receive_msg(self):
        ...
        try:
            ...
            # No message received — return empty string
            return ""
...
class MsgReceiver:
    ...
    def receive_robot_sensor_signal(self, last_robot_sensor_signal):
        ...
        if receive_ret and packed_receive_msg != "":
            unpack_ret, unpacked_receive_msg = self.unpack_msg(packed_receive_msg)

            if unpack_ret and isinstance(unpacked_receive_msg, list):
                robot_sensor_signal = RobotSensorSignal(unpacked_receive_msg) 
    ...
    def get_pose_estimate(self):
        ...
        if ids is not None:
            ...
            for i in range(len(ids)):
                ...
                marker_length = parameters.marker_length  # in meters

                # Define 3D marker corner coordinates
                obj_points = np.array([
                ...
                # Solve pose
                success, rvec, tvec = cv2.solvePnP(obj_points, image_points, parameters.camera_matrix, parameters.dist_coeffs)
